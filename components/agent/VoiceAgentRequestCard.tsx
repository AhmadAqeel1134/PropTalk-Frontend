...
export default function VoiceAgentRequestCard() {
  ...
  if (!request) {
    return (
      <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 border border-gray-800 hover:border-gray-700 transition-all duration-500 p-8">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-xl font-semibold text-white">Voice Agent Access</h3>
              <p className="text-sm text-gray-400">Get your AI-powered calling assistant</p>
            </div>
          </div>
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 border border-gray-800 hover:border-gray-700 transition-all duration-500">
      <div className={`absolute inset-0 bg-gradient-to-br ${config.gradient} opacity-5 group-hover:opacity-10 transition-opacity duration-500`}></div>
      
      {request.status === 'pending' && (
        <div className="absolute inset-0">
          <div className="absolute top-0 right-0 w-32 h-32 bg-yellow-500/20 rounded-full blur-3xl animate-pulse"></div>
        </div>
      )}
      ...
      <div className="relative z-10 p-8">
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className={`p-4 rounded-2xl ${config.bg} border ${config.border} backdrop-blur-sm ${request.status === 'pending' ? 'animate-pulse' : ''}`}>
        ...
        <button className="w-full px-6 py-3 rounded-xl bg-gray-800 border border-gray-700 hover:border-gray-600 text-gray-300 hover:text-white font-medium transition-all duration-300 hover:bg-gray-750 group/btn">
