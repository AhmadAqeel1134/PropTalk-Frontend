...
import { X, MapPin, Home, Phone, User, DollarSign, Bed, Bath, Square, FileText, Loader2, Building } from 'lucide-react'
import { useCreateProperty, useMyContacts } from '@/hooks/useAgent'
...
export default function CreatePropertyForm({ onClose, redirectToProperty = true }: CreatePropertyFormProps) {
  ...
  const onSubmit = (data: FormData) => {
      ...
      onSuccess: (newProperty) => {
        if (redirectToProperty && newProperty?.id) {
          router.push(`/agent/properties/${newProperty.id}`)
        } else {
          onClose()
        }
      }
  ...
  return (
    <div
      className={`fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto p-4 ${
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className={`text-sm mt-1 ${
              ...
              Add property to your inventory
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <textarea 
                {...register('amenities')} 
                rows={2} 
                placeholder="Amenities (comma-separated: pool, garage, garden, etc.)" 
                className={inputClass(false)}
              />
            </div>
          </div>
